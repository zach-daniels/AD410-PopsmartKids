...
{//if POST is set, prepare to process form data
	...
	$sql = sprintf("select AdminID,FirstName,Privilege,NumLogins from retro_Admin WHERE Email='%s' AND AdminPW = SHA('%s')",$Email,$MyPass);
	...
	{# valid user, create session vars, redirect!
		...
		$sql = sprintf("UPDATE retro_Admin set NumLogins='%d' WHERE AdminID='%d'",$NumLogins,$AdminID);
